...
const gkdFp = process.cwd() + '/dist/ym.json5';
const versionFp = process.cwd() + '/dist/ym.version.json';
...
export const validSnapshotUrl = (s: string) => {
    const u = new URL(s);
    return u.pathname.startsWith('/i/') || u.pathname.startsWith('/snapshot/');
};
...
export const updateReadMeMd = async (
    ...
    await Promise.all(newConfig.apps!.map(async (app) => {
            debugger
